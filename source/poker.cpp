...
std::vector<int> Poker::dupRankInd(const Player *plyr) const {
    ...
    std::vector<int> handRanks = getRankInd(plyr);         // array pointing to first element in handRanks
...
// todo :: refactor this
std::vector<int> Poker::getRankInd(const Player *plyr) const {
    /*returns a pointer to an array containing rank indexes of each card
     * Typically array is 5 elements*/
    std::vector<int> rankInds(5);                  // will need to be deleted when not used
    int n = 0;
...
bool Poker::hasTwoPair(const Player *player) const {
    ...
    std::vector<int> handRanks = getRankInd(player);
